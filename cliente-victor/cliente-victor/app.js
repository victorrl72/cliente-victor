...
let auth        = null;   // cabecera Basic codificada
...
async function handleLogin() {
    ...
    // Verificamos credenciales pidiendo el dashboard (requiere auth)
    try {
...
async function handleSaveNewTask() {
    ...
    const cmd = {
        ...
        completed:   false,
        deadline:    rawDeadline ? rawDeadline.replace("T", "T").slice(0, 19) : null,
        priority:    document.getElementById("tarea-prioridad").value,
        status:      document.getElementById("tarea-status").value,
        categoryId:  document.getElementById("tarea-categoria").value ? parseInt(document.getElementById("tarea-categoria").value) : null
    };
...
async function toggleTask(t) {
    ...
    const cmd = {
        ...
        priority:    t.priority,
        status:      nuevoStatus,
        categoryId:  null  // no cambiamos categoría
    };
...
async function handleModalSave() {
    ...
    const cmd = {
        ...
        deadline:    rawDeadline ? rawDeadline.slice(0, 19) : null,
        priority:    document.getElementById("modal-prioridad").value,
        status:      document.getElementById("modal-status").value,
        categoryId:  document.getElementById("modal-categoria").value ? parseInt(document.getElementById("modal-categoria").value) : null
    };
